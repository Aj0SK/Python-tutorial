...
\section{Úvod}
Predtým ako s Pythonom začneme, je dôležité zodpovedať niekoľko otázok. Prvá a najdôležitejšia je, čo je programovanie. Programovanie je vytváranie programov, ktoré na počítači vieme spustiť a majú nejakú funkciu. Príkladom programu môže byť aplikácia na četovanie, webový prehliadač ale aj niečo jednoduchšie, ako napríklad kalkulačka. O programoch platí, že majú vstup a výstup. Pre program fungujúci ako kalkulačka môže byť vstupom matematický výraz typu $5+3\cdot 7$ a výstupom hodnota tohto výrazu $26$. My sa v tomto tutoriále budeme venovať jednoduchým programom. V prípade kalkulačky na vašom počítači ste zrejme zvyknutí zadať matematiký výraz pomocou tlačidiel, teda nejakého grafického rozhrania (tlačidlá, okná a podobne). Takéto rozhranie naše programy nebudú mať, čo ich mimochodom značne skracuje. Vstup aj výstup bude teda v textovej forme.\\

Keď už vieme čo je program a chceme programovať, potrebujeme vedieť ako taký program vytvoriť. Na to slúžia, ako už isto tušíte, programovacie jazyky. Tie nám dovoľujú vytvoriť postupnosť príkazov, ktoré má daný program vykonať. Po spustení dostane program nejaký vstup a vyprodukuje naň odpoveď (výstup), ktorú typicky nejako zobrazí, napr. vypíše.
...
Nakoniec výraz $7 \% 3$ kde \% označuje zvyšok po delení. Označuje teda tú časť, ktorá ostane po celočíselnom delení. $7//3=2$ a teda ostalo $7 - 2 \cdot 3 = 1$.
...
V druhom riadku vidíme, že podobne ako čísla, reťazce vieme sčítavať. Čo urobí znamienko plus v tomto prípade je, že zlepí reťazce za seba. Ako je možné, že výsledok 3. a 4. riadku sa líši? Dôvodom je, že v prvom prípade ide o plus, . V druhom prípade sme úvodktoré sa aplikuje na čísla, kedže Python vidí, že naľavo aj napravo od plus sa nachádzajú číslazovkami okolo čísel dali najavo, že chceme tieto čísla reprezentovať ako reťazce a Pythonu teda neostáva nič iné, ako použiť plus pre reťazce.
...
% TODO (Žaba): toto som si nie istý, ako má byť použité. Lebo ak ako ťahák po prednáške fajn, ak len tak na čítanie, tak som sa zrovna trochu stratil. Priveľa informácií, primálo príkladov. Napr. typy by som sem neťahal (v tom Pythone obzvlášť). Skôr čo je premenná, meno a hodnota, ako ju vytvoriť, že = sa znamená "priradiť do" (s týmto majú začiatočníci dosť problém pochopiť) a že to nie je obojstranné a možno taký ten pokec, že ako to funguje, že v krabičke je hodnota, keď zavolám to meno, tak sa pozriem do tej krabičky. Typy by som možno na konci spomenul, že toto sa tam najčastejšie pchá, Python to rieši za mňa, je fajn poznať skratky kvôli chybovým hláškam (viď vyššie) a takisto načítavanie.

Program si k svojmu chodu potrebuje často pamätať nejaké informácie. Či už je to vstup, ktorý programu užívateľ zadal alebo nejaké medzivýsledky výpočtu, ktorý program vykonáva. V Pythone na toto slúžia premenné. Premenná je krabička (miesto v pamäti počítača), kde si program ukladá dáta. Každá premenná má svoj
...
Premenné v Pythone netreba pred použitím vytvoriť, stačí konkrétnej premennej priradiť hodnotu a premennú za nás vytvorí počítač. Pomocou \texttt{=} vieme do premennej uložiť novú hodnotu alebo výraz zložený z matematických operátorov a iných premenných.

\lstlang{python}\begin{lstlisting}
a = 4   # do a priradime 4
b = 2 * a - 3   # do b priradim hodnotu vyrazu napravo, teda 5
c, d = 10, 12   # vieme robit aj viac priradeni naraz (c = 10 a d = 12)
\end{lstlisting}

%%%% TODO: kecy o ==, operatoroch a co vracaju
...
Čakali by sme, že sa $a$ nezmení avšak tentokrát je $a$ \texttt{list}. Prečo sa v tomto prípade zmenila hodnota $a$? Je to z dôvodu, že do funkcie bola v tomto prípade poslaná nie kópia listu ale priamo list $a$. Je trochu nelogické, prečo sa python správa rôzne pri rôznych typoch. Je to napríklad z dôvôdu, že listy sú zväčša pomerne veľké, zaberajú viac miesta a teda aj ich kopírovanie je pomalé a Python sa mu snaží vyhnúť. Je dôležité si na toto správanie dávať pozor a keď je to dôležité, vyskúšať ako sa Python správa pre konkrétny typ. Pokiaľ by sme vyskúšali vyššie uvedený program napríklad pre reťazec, zistili by sme, že spôsobí rovnaké správanie ako keď je parametrom číslo, teda Python reťazce kopíruje.
